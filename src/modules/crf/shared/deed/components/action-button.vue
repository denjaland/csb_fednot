<template>
  ...
  <div>
    ...
    <div class="absolute right-0 bg-white rounded shadow overflow-hidden" v-if="state.open">
        <div v-on:click="correct()" class="  py-2 px-4  font-semibold "
          :class="{
            'cursor-pointer hover:bg-gray-50 text-indigo-600': !has_pending_changes,
            'cursor-not-allowed text-gray-300': has_pending_changes
          }"
        >{{$t('correct')}}</div>
        <!--div v-on:click="cancel()" class="  py-2 px-4  font-semibold "
        :class="{
            'cursor-pointer hover:bg-gray-50 text-indigo-600': !has_pending_changes,
            'cursor-not-allowed text-gray-300': has_pending_changes
          }">{{$t('cancel')}}</div-->
    ...
    <div class="absolute right-0 bg-white rounded shadow overflow-hidden text-indigo-600" v-if="state.open">
        ...
        <div v-on:click="correct()" class="  py-2 px-4  font-semibold flex flex-row items-center"
        :class="{
            'cursor-pointer hover:bg-gray-50': !has_pending_changes,
            'cursor-not-allowed text-gray-300': has_pending_changes
          }">
          <icon-correct class="w-6 h-6 mr-4"></icon-correct>
          <div>{{$t('correct')}}</div>
        </div>
        <!--div v-on:click="cancel()" class="  py-2 px-4  font-semibold flex flex-row items-center"
        :class="{
            'cursor-pointer hover:bg-gray-50': !has_pending_changes,
            'cursor-not-allowed text-gray-300': has_pending_changes
          }">
          <icon-delete class="w-6 h-6 mr-4"></icon-delete>
          <div>{{$t('cancel')}}</div>
        </div-->
    </div>
    </div>
...
<div class="relative" v-click-outside="close">
    ...
    <tooltip-component class="relative mr-4"  :tooltip="$t('correct')" >
        <div v-on:click="correct()"><icon-correct class="w-6 h-6 transition-all"
          :class="{
            'cursor-pointer text-gray-400 hover:text-indigo-600': !has_pending_changes,
            'cursor-not-allowed text-gray-300': has_pending_changes
          }"
        ></icon-correct></div>
     </tooltip-component>
     <!--tooltip-component class="relative mr-4"  :tooltip="$t('cancel')" >
        <div v-on:click="cancel()"><icon-delete class="w-6 h-6 transition-all"
        :class="{
            'cursor-pointer text-gray-400 hover:text-indigo-600': !has_pending_changes,
            'cursor-not-allowed text-gray-300': has_pending_changes
          }"></icon-delete></div>
...
import IconCorrect from '../../../../../components/icons/correct';
...
export default {
  ...
  computed: {
    has_pending_changes() {
      return _.filter(this.deed.staging, function(s) { return s.status === 'pending'}).length > 0;
    }
  },
  methods: {
    close: function() {
    ...
    correct() {
      if(this.has_pending_changes) return false;
      this.$router.push('/' + this.$route.params.locale + '/crf/registrations/' + this.deed.deed_id + '/edit');
    },
    cancel() {
      if(this.has_pending_changes) return false;
      alert('cancel goes here')
    },
    pdf() {
      let pdf = new jsPDF();
